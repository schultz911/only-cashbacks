...
## 2026-06-03 - Prevent Server DoS via Unbounded External Fetch Calls
**Vulnerability:** The server `fetch` call to the OpenRouter API lacked a timeout configuration. If the external API became unresponsive, the request would hang indefinitely. Attackers could exploit this by triggering many requests, exhausting all server connections/memory and causing a Denial of Service (DoS).
**Learning:** Node.js native `fetch` does not have a default timeout. External API calls must always explicitly define a timeout mechanism (e.g. `AbortSignal.timeout()`).
**Prevention:** Always add a reasonable timeout using `AbortSignal.timeout(ms)` to any server-side `fetch` request calling external services.

## 2026-06-03 - Prevent OOM DoS via Unbounded In-Memory Rate Limiter
**Vulnerability:** The custom in-memory rate limiter in `server.ts` stored IPs in an unbounded `Map`. While expired entries were periodically cleaned up, an attacker could spoof thousands of unique IPs or distribute requests to rapidly fill the `Map` within the cleanup window, leading to an Out of Memory (OOM) server crash.
**Learning:** Any in-memory cache used for tracking user metrics (like rate limiting) must have a hard boundary to prevent memory exhaustion attacks.
**Prevention:** Implement a maximum size limit on the rate limit cache (e.g., `rateLimitCache.size >= 10000`) and reject new IP entries with a 429 status code if the limit is exceeded.
