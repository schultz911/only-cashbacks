...
function BillReminderItem({ bill, isAnimating, onPay }: BillReminderItemProps) {
  return (
    <motion.div
      layout
      ...
      className={cn(
        ...
        !isAnimating ? "bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700" : "bg-green-50 border-green-300 dark:bg-emerald-900/50 dark:border-emerald-600"
      )}
    >
      {isAnimating ? (
        <motion.div
...
export function BillReminders({ walletCards, cardBillDates, paidBills, markBillPaid }: BillRemindersProps) {
  ...
  const bills = useMemo(() => {
    return calculateBills(walletCards, cardBillDates, paidBills);
        ...
        <AnimatePresence mode="popLayout">
          {bills.map((bill) => (
            <BillReminderItem
              key={bill.card.id}
              bill={bill}
              isAnimating={animatingCardId === bill.card.id}
              onPay={handlePay}
            />
          ))}
        </AnimatePresence>
