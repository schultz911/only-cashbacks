...
export default function App() {
  ...
  // OPTIMIZATION: Memoize the active wallet cards to avoid redundant array allocations, maps and filters on every render
  const activeWalletCards = useMemo(() => {
    return walletCards.length > 0
      ? walletCards.map(id => CARD_DICT[id]).filter(c => c && !c.isDummy)
